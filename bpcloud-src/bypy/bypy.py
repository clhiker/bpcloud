...
"you are not going to send sensitive data to the cloud plainly right?")
	...
	def __replace_list_format(self, fmt, j):
		...
		for k, v in ByPy.ListFormatDict.items():
			self.select2FilesList(k, v(j))
			output = output.replace(k, v(j))
		return output

# ==============================================================================
	# 我添加的一部分
	def select2FilesList(self, k, v):
		if k == '$t':			# type
			self.files_dict[k] = v
		elif k == '$f':			# name
			self.files_dict[k] = v
			...
			self.files_dict[k] = v
		if len(self.files_dict) == 5:
			files_dict_temp = self.files_dict.copy()
			self.fileslist.append(files_dict_temp)
			self.files_dict.clear()

	# 进度获取
	def getDownLoadProcess(self):
		return self.download_process
...
Possible fixes:
 ...
 2. If (1) still doesn't solve the problem, you may have to go to:
	...
	def __verify_current_file(self, j, gotlmd5):
		...
		if self.__current_file_size == rsize or self.state == 'cancel':
	...
	def __list_act(self, r, args):
		...
		for f in j['list']:
			result = self.__replace_list_format(fmt, f)
			# pr(result)
			# self.fileslist.append(result)
...
try to create a file at PCS by combining slices, having MD5s specified
  ...
  md5s - MD5 digests of the slices, can be:
	...
	def __downchunks(self, rfile, start):
		rsize = self.__remote_json['size']			# 这个是文件大小
		...
		start_time = time.time()
		while True:
			nextoffset = offset + self.__dl_chunk_size		# 分段大小

			# 进度获取
			if rsize > 0:
				self.download_process = round(nextoffset / rsize * 100)
			self.download_size = nextoffset
...
#				#pars['ru'] = rangemagic
		...
		# No exception above, then everything goes fine
		result = const.ENoError
		if self.__verify and self.state != 'cancel':
			self.__current_file_size = getfilesize(self.__current_file)
			result = self.__verify_current_file(self.__remote_json, False)
...
'''.format(const.RemoteTempDir).strip()
	...
	# 我添加的获取配置位置
	def getTokenFilePath(self):
		return const.TokenFilePath

	def getSignUrl(self):
		return const.ServerAuthUrl + '?' + pars
